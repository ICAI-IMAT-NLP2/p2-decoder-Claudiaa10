...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        ...
        if mask is not None:
            scores = torch.masked_fill(scores,mask == 0, value = float("-inf"))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, hidden_state, mask=None):
        ...
        x = self.output_linear(
            torch.cat([head(hidden_state,mask) for head in self.heads], dim=-1)
        )
        return x
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        x = self.layer_norm_2(self.feed_forward(self.layer_norm_1(self.self_attention(x,mask))))
        return x
...
class Embeddings(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int):
        ...
        self.layer_norm = nn.LayerNorm(d_model,vocab_size)
